# OpenWRT-Package-update-safe-mode
На время обновления GUI отваливается, но через 2-3 минуты возвращается обратно.
Скрипт нужно прогнать 3 раза. Так как он не сразу все пакеты обновляет, а частями.
